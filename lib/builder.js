  ...
  for (var idx = 0; idx < this.events.length; idx++) {
    ...
    if (_event.repeating) {
      ...
      if (_event.repeating.byday) {
        rrlue += ';BYDAY=' + _event.repeating.byday.toUpperCase();
      }
      lines.push(rrlue);
    }
...
ICSFileBuilder.prototype._checkAndBuildEventObject = function (rawEvent) {
  ...
  if (rawEvent.repeating) {
    ...
    if (rawEvent.repeating.byday) {
      if (!(typeof rawEvent.repeating.byday === 'string')) {
        return new Error('event.repeating.byday must be a String.');
      }
      event.repeating.byday = rawEvent.repeating.byday.toUpperCase();
    }
  }
